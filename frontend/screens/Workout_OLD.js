...
function Workout({ navigation }) {
    ...
    const finishWorkout = useCallback(async () => {
        ...
        // 🔔 NEW: publish a pulse for ActivityChips
        try {
            const tplName = getTemplateName(completedWorkoutData.tid) || "workout";
            await addDoc(collection(db, "users", global.userData.uid, "pulse"), {
                type: "workout",
                templateName: tplName,
                ts: serverTimestamp(),
                // optional context that your chips/hook may use
                uid: global.userData.uid,
                handle: global.userData?.handle || "",
                name: global.userData?.name || "",
                pfpVersion: global.userData?.pfpVersion || 0,
                workoutId: completedWorkoutData.wid,
            });
        } catch (e) {
            console.log("post workout pulse error", e);
        }
    ...
    // resolve template name by tid (uses local state first, then global fallback)
    const getTemplateName = React.useCallback((tid) => {
        if (!tid) return null;
        const list =
            (Array.isArray(templates) && templates.length ? templates : (global.userData?.templates || []));
        const found = list.find(t => t?.tid === tid);
