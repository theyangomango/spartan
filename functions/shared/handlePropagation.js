...
export async function findUserByHandle(rawHandle) {
  ...
  for (const { field, value } of candidates) {
    ...
    const snapshot = await getDb().collection(PRIMARY_USER_COLLECTION).where(field, "==", value).limit(2).get();
  ...
  const handleDoc = await getDb().collection("userHandles").doc(lower).get();
  if (handleDoc.exists) {
    const uid = String(handleDoc.data()?.uid || "").trim();
    if (uid) {
      const direct = await getDb().collection(PRIMARY_USER_COLLECTION).doc(uid).get();
      if (direct.exists) {
        return direct;
      }
    }
...
export async function ensureHandleAvailable(newHandle, uid) {
  ...
  const handleSnap = await getDb().collection("userHandles").doc(lower).get();
  ...
  for (const { field, value } of candidates) {
    if (!value || tried.has(`${field}:${value}`)) continue;
    tried.add(`${field}:${value}`);
    const snapshot = await getDb().collection(PRIMARY_USER_COLLECTION).where(field, "==", value).limit(1).get();
    if (snapshot.empty) continue;
...
export async function propagateHandleChange({ uid, oldHandle, newHandle }) {
  const current = normaliseHandle(oldHandle);
  const next = normaliseHandle(newHandle);
  if (!uid || !current || !next) {
    console.log("propagateHandleChange: missing uid/handles, skipping propagation.");
    return;
  }
  if (current.toLowerCase() === next.toLowerCase()) {
    console.log("propagateHandleChange: handles match (case-insensitive), skipping propagation.");
    return;
  }

  replaceConfig = {
    oldHandle: current,
    oldHandleLower: current.toLowerCase(),
    newHandle: next,
    newHandleLower: next.toLowerCase(),
    oldHandleAt: `@${current}`,
    newHandleAt: `@${next}`,
  };

  console.log(`propagateHandleChange: updating handle references for uid ${uid}`);
