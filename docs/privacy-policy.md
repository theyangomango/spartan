...
- Usage and device data: device type, OS version, app version, general engagement events.
...
- Within the Service:
  ...
  - Direct messages: designed to be private between participants. If end‑to‑end encryption is enabled, only the participants can read message content; otherwise messages are encrypted in transit and at rest and may be accessible to us or our processors for security, abuse handling, or as required by law. Metadata (e.g., time sent, participants) may be processed for delivery and safety.
