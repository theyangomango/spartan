...
// Cross-environment hexagon computation shared between the client and backend.
// Mirrors the backend logic while remaining Metro-friendly (no cross-directory imports).
...
export function computeHexagonFromStats({ statsExercises = {}, prevStatsHexagon = {}, trainedExerciseNames = [] } = {}) {
  ...
  Object.keys(statsExercises || {}).forEach((exName) => {
    ...
    if (!timeline.length) {
      const sets = Array.isArray(entry?.recentSets) ? entry.recentSets : [];
      for (const s of sets) {
  ...
  return {
    ...
    lastTrained: {
      shoulders: Number.isFinite(lastTs.shoulders) ? lastTs.shoulders : 0,
      chest: Number.isFinite(lastTs.chest) ? lastTs.chest : 0,
      arms: Number.isFinite(lastTs.arms) ? lastTs.arms : 0,
      legs: Number.isFinite(lastTs.legs) ? lastTs.legs : 0,
      back: Number.isFinite(lastTs.back) ? lastTs.back : 0,
      abs: Number.isFinite(lastTs.abs) ? lastTs.abs : 0,
    },
  };
}
